...
CWSDLMessage * CWSDLPortTypeIO::GetMessage()
{
	...
	if (pDoc != NULL)
	{
				...
				if (pDoc->GetDocumentType() == WSDLDOC)
				{
					...
					if (m_pMessage == NULL)
					{
						EmitFileError(IDS_SDL_UNRESOLVED_ELEM2, const_cast<CWSDLPortTypeIO *>(this), 0,
							"message", static_cast<LPCWSTR>(strUri), static_cast<LPCWSTR>(m_message.GetName()));
					}
				}
			}
		}
		...
		{
			EmitFileError(IDS_SDL_UNRESOLVED_NAMESPACE, const_cast<CWSDLPortTypeIO *>(this), 0, static_cast<LPCWSTR>(m_message.GetPrefix()));
		}
	}
