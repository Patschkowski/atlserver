...
{
	ATLENSURE(lpszMessage && lpszStackTrace);
	...
	TCHAR szBuf[256];
	_stprintf_s(szBuf, sizeof(szBuf)/sizeof(TCHAR), _T("%d\0"), dwProcessId);
	m_ProcessEdit = szBuf;
...
void CDebugDialog::OnCopy() 
{
	...
	if (OpenClipboard())
	{
		...
		_tcscpy_s(lpszData, (str.GetLength()+1)/sizeof(TCHAR), ((LPCTSTR)str));
