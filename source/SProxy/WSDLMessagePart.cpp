...
HRESULT CWSDLMessagePart::GetType(CXSDElement **ppElement, XSDTYPE *pXSD)
{
					...
					(strUri == CLR_TYPES_NAMESPACEW))
				{
					...
					*pXSD = XSDTYPE_STRING;
					EmitFileWarning(IDS_SDL_CUSTOM_TYPE, const_cast<CWSDLMessagePart *>(this), 0,
						static_cast<LPCWSTR>(strUri), static_cast<LPCWSTR>(m_type.GetName()));
				}
				else
				{
					//
	...
	*ppElement = m_pXSDElement;
	...
	{
		EmitFileError(IDS_SDL_UNRESOLVED_NAMESPACE, const_cast<CWSDLMessagePart *>(this), 0, static_cast<LPCWSTR>(m_type.GetPrefix()));
	}
	else
	{
		EmitFileError(IDS_SDL_UNRESOLVED_ELEM2, const_cast<CWSDLMessagePart *>(this), 0, "part", static_cast<LPCWSTR>(strUri), static_cast<LPCWSTR>(m_type.GetName()));
	}
	return E_FAIL;
}